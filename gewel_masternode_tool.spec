...
# See https://github.com/gewelio/trezor-firmware/commit/cff481b842550f8ac8f85a31044c723a773925dc
...
a = Analysis(['src/gewel_masternode_tool.py'],
             pathex=[base_dir],
...
exe = EXE(pyz,
          ...
          a.zipfiles,
          a.datas,
          name='GewelMasternodeTool',
          debug=False,
          strip=False,
          upx=False,
          console=False,
		  icon=os.path.join('img',('gmt.%s' % ('icns' if os_type=='darwin' else 'ico'))))

if os_type == 'darwin':
    app = BUNDLE(exe,
                 name='GewelMasternodeTool.app',
                 icon='img/gmt.icns',
...
if os_type == 'win32':
    ...
    os.system('"7z.exe" a %s %s -mx0' % (os.path.join(all_bin_dir, 'GewelMasternodeTool_' + version_str + '.win' + no_bits + '.zip'),  'GewelMasternodeTool.exe'))
elif os_type == 'darwin':
    print('Compressing Mac executable')
    os.system('zip -r "%s" "%s"' % (os.path.join(all_bin_dir, 'GewelMasternodeTool_' + version_str + '.mac.zip'),  'GewelMasternodeTool.app'))
elif os_type == 'linux':
    print('Compressing Linux executable')
    os.system('tar -zcvf %s %s' % (os.path.join(all_bin_dir, 'GewelMasternodeTool_' + version_str + '.linux.tar.gz'),  'GewelMasternodeTool'))
