...
from geweld_intf import GeweldInterface
...
log = logging.getLogger('gmt.reg_masternode')


class RegMasternodeDlg(QDialog, ui_reg_masternode_dlg.Ui_RegMasternodeDlg, WndUtils):
    def __init__(self, main_dlg, config: AppConfig, geweld_intf: GeweldInterface, masternode: MasternodeConfig,
        ...
        self.deterministic_mns_spork_active = self.app_config.is_spork_15_active(self.geweld_intf)
        self.gmn_collateral_tx: str = None
        self.gmn_collateral_tx_index: int = None
        self.gmn_collateral_tx_address: str = None
        self.gmn_collateral_tx_address_path: str = None
        self.gmn_ip: str = None
        self.gmn_tcp_port: int = None
        self.gmn_owner_payout_addr: str = None
        self.gmn_operator_reward: int = 0
        self.gmn_owner_privkey: str = None
        self.gmn_owner_address: str = None
        self.gmn_operator_privkey: str = None
        self.gmn_operator_pubkey: str = None
        self.gmn_voting_privkey: str = None
        self.gmn_voting_address: str = None
        self.gmn_owner_key_type = InputKeyType.PRIVATE
        self.gmn_operator_key_type = InputKeyType.PRIVATE
        self.gmn_voting_key_type = InputKeyType.PRIVATE
        ...
        self.summary_info = []
        if self.masternode:
            self.gmn_collateral_tx_address_path = self.masternode.collateralBip32Path
    ...
    def setupUi(self):
        ...
        self.setIcon(self.btnSummaryGMNOperatorKeyCopy, 'content-copy@16px.png')
        self.edtSummaryGMNOperatorKey.setStyleSheet("QLineEdit{background-color: white} "
    ...
    def update_dynamic_labels(self):
        ...
        def get_label_text(prefix:str, key_type: str, tooltip_anchor: str, style: str):
            lbl = prefix + ' ' + \
                  {'privkey': 'private key', 'pubkey': 'public key', 'address': 'Gewel address'}.get(key_type, '???')
        ...
        if self.masternode:
            ...
            if self.gmn_operator_key_type == InputKeyType.PRIVATE:
            ...
            if self.gmn_voting_key_type == InputKeyType.PRIVATE:
    ...
    def on_lblOwnerKey_linkActivated(self, link):
        if self.gmn_owner_key_type == InputKeyType.PRIVATE:
            self.gmn_owner_key_type = InputKeyType.PUBLIC
            self.gmn_owner_privkey = self.edtOwnerKey.text()
            self.edtOwnerKey.setText(self.gmn_owner_address)
        else:
            self.gmn_owner_key_type = InputKeyType.PRIVATE
            self.gmn_owner_address = self.edtOwnerKey.text()
            self.edtOwnerKey.setText(self.gmn_owner_privkey)
        self.update_dynamic_labels()
        self.update_ctrls_visibility()
    ...
    def on_lblOperatorKey_linkActivated(self, link):
        if self.gmn_operator_key_type == InputKeyType.PRIVATE:
            self.gmn_operator_key_type = InputKeyType.PUBLIC
            self.gmn_operator_privkey = self.edtOperatorKey.text()
            self.edtOperatorKey.setText(self.gmn_operator_pubkey)
        else:
            self.gmn_operator_key_type = InputKeyType.PRIVATE
            self.gmn_operator_pubkey = self.edtOperatorKey.text()
            self.edtOperatorKey.setText(self.gmn_operator_privkey)
        self.update_dynamic_labels()
        self.update_ctrls_visibility()
    ...
    def on_lblVotingKey_linkActivated(self, link):
        if self.gmn_voting_key_type == InputKeyType.PRIVATE:
            self.gmn_voting_key_type = InputKeyType.PUBLIC
            self.gmn_voting_privkey = self.edtVotingKey.text()
            self.edtVotingKey.setText(self.gmn_voting_address)
        else:
            self.gmn_voting_key_type = InputKeyType.PRIVATE
            self.gmn_voting_address = self.edtVotingKey.text()
            self.edtVotingKey.setText(self.gmn_voting_privkey)
        self.update_dynamic_labels()
        self.update_ctrls_visibility()
    ...
    def prepare_keys(self):
        ...
        protx_state = {}
        try:
            for protx in self.geweld_intf.protx('list', 'registered', True):
        ...
        if found_protx:
            if self.masternode.get_gmn_owner_public_address(self.app_config.gewel_network) == \
                    protx_state.get('ownerAddress'):
                gen_owner = True

            if self.masternode.get_gmn_operator_pubkey() == protx_state.get('pubKeyOperator'):
                gen_operator = True

            if self.masternode.get_gmn_voting_public_address(self.app_config.gewel_network) == \
                    protx_state.get('votingAddress'):
                gen_voting = True

        if (self.masternode.gmn_owner_key_type == InputKeyType.PRIVATE and
             not self.masternode.gmn_owner_private_key) or \
                (self.masternode.gmn_owner_key_type == InputKeyType.PUBLIC and
                 not self.masternode.gmn_owner_address):
            gen_owner = True

        if (self.masternode.gmn_operator_key_type == InputKeyType.PRIVATE and
            not self.masternode.gmn_operator_private_key) or \
                (self.masternode.gmn_operator_key_type == InputKeyType.PUBLIC and
                not self.masternode.gmn_operator_public_key):
            gen_operator = True

        if (self.masternode.gmn_voting_key_type == InputKeyType.PRIVATE and
            not self.masternode.gmn_voting_private_key) or \
                (self.masternode.gmn_voting_key_type == InputKeyType.PUBLIC and
                 not self.masternode.gmn_voting_address):
            gen_voting = True

        if gen_owner:
            self.owner_pkey_generated =  generate_wif_privkey(self.app_config.gewel_network, compressed=True)
            ...
            self.gmn_owner_key_type = self.masternode.gmn_owner_key_type
        ...
        if gen_operator:
            ...
            self.gmn_operator_key_type = self.masternode.gmn_operator_key_type

        if self.deterministic_mns_spork_active:
            if gen_voting:
                self.voting_pkey_generated = generate_wif_privkey(self.app_config.gewel_network, compressed=True)
    ...
    def on_btnGenerateOwnerKey_clicked(self, active):
        k = generate_wif_privkey(self.app_config.gewel_network, compressed=True)
    ...
    def update_ctrls_visibility(self):
        if not self.deterministic_mns_spork_active:
            ...
            self.btnGenerateVotingKey.setVisible(self.gmn_voting_key_type == InputKeyType.PRIVATE)

        self.btnGenerateOwnerKey.setVisible(self.gmn_owner_key_type == InputKeyType.PRIVATE)
        self.btnGenerateOperatorKey.setVisible(self.gmn_operator_key_type == InputKeyType.PRIVATE)
            ...
            if self.show_field_hinds:
                if self.edtIP.text().strip():
                    ...
                    msg = 'If don\'t set the IP address and port fields, the masternode operator will ' \
                          'have to issue a ProUpServTx transaction using Gewel wallet.'
                    style = 'warning'
    ...
    def upd_payout_addr_info(self, show_invalid_data_msg: bool):
        ...
        if show_invalid_data_msg and self.payout_address_validation_err_msg:
            ...
            if self.show_field_hinds:
                msg = 'The owner\'s payout address can be set to any valid Gewel address - it no longer ' \
        ...
        if show_invalid_data_msg and self.owner_key_validation_err_msg:
            ...
            if self.show_field_hinds:
                if self.gmn_owner_key_type == InputKeyType.PRIVATE:
                    ...
                    msg = 'You can use Gewel address if the related private key is stored elsewhere, eg in ' \
                          'the Gewel Core wallet.<br><span class="warning">Note, that if you provide an address ' \
            ...
            if show_invalid_data_msg and self.voting_key_validation_err_msg:
                ...
                if self.show_field_hinds:
                    if self.gmn_voting_key_type == InputKeyType.PRIVATE:
                        ...
                        msg = 'You can use Gewel address if the related private key is stored elsewhere, eg in ' \
                              'the Gewel Core wallet.<br><span class="warning">Note, that providing an address instead of ' \
    ...
    def get_gewel_node_type(self):
        if self.rbGMTGewelNodeType.isChecked():
            return NODE_TYPE_PUBLIC_RPC
        elif self.rbOwnGewelNodeType.isChecked():
            return NODE_TYPE_OWN
        else:
            return None

    def upd_node_type_info(self):
        nt = self.get_gewel_node_type()
        msg = ''
        if nt is None:
            msg = 'DIP-3 masternode registration involves sending a special transaction via the v0.13 Gewel node ' \
                  '(eg Gewel-Qt). <b>Note, that this requires incurring a certain transaction fee, as with any ' \
            ...
            msg = 'The ProRegTx transaction will be processed via the remote RPC node stored in the app configuration.' \
                  ...
                  'so use this method only if you trust the operator of that node (nodes <i>alice(luna, suzy).gewel-masternode-tool.org</i> are maintained by the author of this application).<br><br>' \
                  '<b>Note 2:</b> if the operation fails (e.g. due to a lack of funds), choose the manual method ' \
                  'using your own Gewel wallet.'

        elif nt == NODE_TYPE_OWN:
            msg = 'A Gewel Core wallet (v0.13) with sufficient funds to cover transaction fees is required to ' \
                  'complete the next steps.'
        self.lblGewelNodeTypeMessage.setText(msg)
    ...
    def update_step_tab_ui(self):
        ...
        if self.current_step == STEP_MN_DATA:
            ...
            if self.gmn_owner_key_type == InputKeyType.PRIVATE:
                owner_privkey = self.gmn_owner_privkey
            else:
                owner_privkey = '&lt;not available&gt;'

            if self.gmn_operator_key_type == InputKeyType.PRIVATE:
                operator_privkey = self.gmn_operator_privkey
            else:
                operator_privkey = '&lt;not available&gt;'

            if self.gmn_voting_key_type == InputKeyType.PRIVATE:
                voting_privkey = self.gmn_voting_privkey
            else:
                voting_privkey = '&lt;not available&gt;'

            self.summary_info = \
                [f'Network address\t{self.gmn_ip}:{self.gmn_tcp_port}',
                 f'Payout address\t{self.gmn_owner_payout_addr}',
                 f'Owner private key\t{owner_privkey}',
                 f'Owner public address\t{self.gmn_owner_address}',
                 f'Operator private key\t{operator_privkey}',
                 f'Operator public key\t{self.gmn_operator_pubkey}',
                 f'Voting private key\t{voting_privkey}',
                 f'Voting public address\t{self.gmn_voting_address}',
                 f'Deterministic MN tx hash\t{self.gmn_reg_tx_hash}']
            ...
            if self.gmn_operator_key_type == InputKeyType.PRIVATE:
                operator_message = '<b><span style="color:red">One more thing... <span></b>copy the following ' \
                                   'line to the <code>gewel.conf</code> file on your masternode server ' \
                                   '(and restart <i>geweld</i>) or pass it to the masternode operator:'
            else:
                operator_message = '<b><span style="color:red">One more thing... <span></b>copy the following ' \
                                   'line to the <code>gewel.conf</code> file on your masternode server, replacing ' \
            ...
            self.edtSummaryGMNOperatorKey.setText(f'masternodeblsprivkey={operator_privkey}')
            self.btnCancel.hide()
            self.btnBack.hide()
    ...
    def validate_data(self):
        self.gmn_collateral_tx = self.edtCollateralTx.text().strip()
        self.collateral_validation_err_msg = ''
        error_count = 0
        try:
            if not self.gmn_collateral_tx:
                self.collateral_validation_err_msg = 'Collteral transaction ID is required.'
                self.edtCollateralTx.setFocus()
            else:
                self.gmn_collateral_tx_index = int(self.edtCollateralIndex.text())
                if self.gmn_collateral_tx_index < 0:
        ...
        self.ip_port_validation_err_msg = ''
        try:
            self.gmn_ip = self.edtIP.text().strip()
            if self.gmn_ip:
                ipaddress.ip_address(self.gmn_ip)
        except Exception as e:
            self.edtIP.setFocus()
        ...
        try:
            if self.gmn_ip:
                self.gmn_tcp_port = int(self.edtPort.text())
            else:
                self.gmn_tcp_port = None
        except Exception:
        ...
        if not addr:
            ...
            self.gmn_owner_payout_addr = addr
            if not validate_address(self.gmn_owner_payout_addr, self.app_config.gewel_network):
        ...
        self.operator_reward_validation_err_msg = ''
        if self.chbWholeMNReward.isChecked():
            self.gmn_operator_reward = 0
        else:
            self.gmn_operator_reward = self.edtOperatorReward.value()
            if self.gmn_operator_reward > 100 or self.gmn_operator_reward < 0:
        ...
        if not key:
            ...
            if self.gmn_owner_key_type == InputKeyType.PRIVATE:
                self.gmn_owner_privkey = key
                if not validate_wif_privkey(self.gmn_owner_privkey, self.app_config.gewel_network):
                    self.edtOwnerKey.setFocus()
                    self.owner_key_validation_err_msg = 'Invalid owner private key.'
                else:
                    self.gmn_owner_address = wif_privkey_to_address(self.gmn_owner_privkey, self.app_config.gewel_network)
            else:
                self.gmn_owner_address = key
                self.gmn_owner_privkey = ''
                if not validate_address(self.gmn_owner_address, self.app_config.gewel_network):
                    self.edtOwnerKey.setFocus()
                    self.owner_key_validation_err_msg = 'Invalid owner Gewel address.'
        ...
        if not key:
            ...
            if self.gmn_operator_key_type == InputKeyType.PRIVATE:
                try:
                    self.gmn_operator_privkey = key

                    try:
                        b = bytes.fromhex(self.gmn_operator_privkey)
                        if len(b) != 32:
                    ...
                    self.gmn_operator_pubkey = bls_privkey_to_pubkey(self.gmn_operator_privkey)
                except Exception as e:
                    self.edtOperatorKey.setFocus()
                    self.operator_key_validation_err_msg = 'Invalid operator private key: ' + str(e)
            else:
                self.gmn_operator_pubkey = key
                self.gmn_operator_privkey = ''
                try:
                    b = bytes.fromhex(self.gmn_operator_pubkey)
                    if len(b) != 48:
                        raise Exception('invalid length (' + str(len(b)) + ')')
                except Exception as e:
                    self.edtOperatorKey.setFocus()
        ...
        if self.deterministic_mns_spork_active:
            ...
            if not key:
                ...
                if self.gmn_voting_key_type == InputKeyType.PRIVATE:
                    self.gmn_voting_privkey = key
                    if not validate_wif_privkey(self.gmn_voting_privkey, self.app_config.gewel_network):
                        self.edtVotingKey.setFocus()
                        self.voting_key_validation_err_msg = 'Invalid voting private key.'
                    else:
                        self.gmn_voting_address = wif_privkey_to_address(self.gmn_voting_privkey, self.app_config.gewel_network)
                else:
                    self.gmn_voting_address = key
                    self.gmn_voting_privkey = ''
                    if not validate_address(self.gmn_voting_address, self.app_config.gewel_network):
                        self.edtVotingKey.setFocus()
                        self.voting_key_validation_err_msg = 'Invalid voting Gewel address.'
        else:
            # spork 15 not active - use the owner private key for voting
            self.gmn_voting_address = self.gmn_owner_address
            self.gmn_voting_privkey = self.gmn_owner_privkey
            self.gmn_voting_key_type = self.gmn_owner_key_type
    ...
    def get_collateral_tx_address_thread(self, ctrl: CtrlObject, check_break_scanning_ext: Callable[[], bool]):
        ...
        try:
            tx = self.geweld_intf.getrawtransaction(self.gmn_collateral_tx, 1, skip_cache=True)
        except Exception as e:
            raise Exception('Cannot get the collateral transaction due to the following errror: ' + str(e))

        vouts = tx.get('vout')
        if vouts:
            if self.gmn_collateral_tx_index < len(vouts):
                vout = vouts[self.gmn_collateral_tx_index]
                spk = vout.get('scriptPubKey')
                if not spk:
                    raise Exception(f'The collateral transaction ({self.gmn_collateral_tx}) output '
                                    f'({self.gmn_collateral_tx_index}) doesn\'t have value in the scriptPubKey '
                                    f'field.')
                ...
                raise Exception(f'Transaction {self.gmn_collateral_tx} doesn\'t have output with index: '
                                f'{self.gmn_collateral_tx_index}')
        else:
        ...
        if self.gmn_collateral_tx_address_path:
            try:
                addr = hw_intf.get_address(self.main_dlg.hw_session, self.gmn_collateral_tx_address_path)
            except CancelException:
                return False

            msg = ''
            if addr != self.gmn_collateral_tx_address:
                log.warning(
                    f'The address returned by the hardware wallet ({addr}) for the BIP32 path '
                    f'{self.gmn_collateral_tx_address_path} differs from the address stored the mn configuration '
                    f'(self.gmn_collateral_tx_address). Need to scan wallet for a correct BIP32 path.')

                msg = '<span style="color:red">The BIP32 path of the collateral address from your mn config is incorret.<br></span>' \
                      f'Trying to find the BIP32 path of the address {self.gmn_collateral_tx_address} in your wallet.' \
                      f'<br>This may take a while (<a href="break">break</a>)...'
                self.gmn_collateral_tx_address_path = ''
        else:
            msg = 'Looking for a BIP32 path of the Gewel address related to the masternode collateral.<br>' \
                  'This may take a while (<a href="break">break</a>)....'

        if not self.gmn_collateral_tx_address_path and not self.finishing:
            ...
            addr = self.bip44_wallet.scan_wallet_for_address(self.gmn_collateral_tx_address, check_break_scanning,
                                                             fetch_txes_feeback)
            if not addr:
                if not break_scanning:
                    WndUtils.errorMsg(f'Couldn\'t find a BIP32 path of the collateral address ({self.gmn_collateral_tx_address}).')
                return False
            else:
                self.gmn_collateral_tx_address_path = addr.bip32_path
    ...
    def next_step(self):
        ...
        if self.current_step == STEP_MN_DATA:
            ...
            self.gmn_reg_tx_hash = self.edtManualTxHash.text().strip()
            if not self.gmn_reg_tx_hash:
                self.edtManualTxHash.setFocus()
                self.errorMsg('Invalid transaction hash.')
                return
            try:
                bytes.fromhex(self.gmn_reg_tx_hash)
            except Exception:
    ...
    def proregtx_automatic_thread(self, ctrl):
        ...
        try:
            ...
            try:
                ...
                if not self.geweld_intf.is_current_connection_public():
                    try:
                        ...
                        self.geweld_intf.disable_conf_switching()
                ...
                if self.gmn_owner_key_type == InputKeyType.PRIVATE:
                    owner_key = self.gmn_owner_privkey
                else:
                    owner_key = self.gmn_owner_address

                params = ['register_prepare', self.gmn_collateral_tx, self.gmn_collateral_tx_index,
                          self.gmn_ip + ':' + str(self.gmn_tcp_port) if self.gmn_ip else '0', owner_key,
                          self.gmn_operator_pubkey, self.gmn_voting_address, str(round(self.gmn_operator_reward, 2)),
                          self.gmn_owner_payout_addr]
                if funding_address:
                    params.append(funding_address)
                call_ret = self.geweld_intf.protx(*params)
            ...
            try:
                self.gmn_reg_tx_hash = self.geweld_intf.protx('register_submit', protx_tx, payload_sig_str)
                # self.gmn_reg_tx_hash = 'dfb396d84373b305f7186984a969f92469d66c58b02fb3269a2ac8b67247dfe3'
                log.debug('protx register_submit returned: ' + str(self.gmn_reg_tx_hash))
            ...
            self.geweld_intf.enable_conf_switching()
    ...
    def update_manual_protx_prepare_command(self):
        ...
        if addr:
            valid = validate_address(addr, self.app_config.gewel_network)
            if valid:
                if self.gmn_owner_key_type == InputKeyType.PRIVATE:
                    owner_key = self.gmn_owner_privkey
                else:
                ...
                cmd = f'protx register_prepare "{self.gmn_collateral_tx}" "{self.gmn_collateral_tx_index}" ' \
                    f'"{self.gmn_ip + ":" + str(self.gmn_tcp_port) if self.gmn_ip else "0"}" ' \
                    f'"{owner_key}" "{self.gmn_operator_pubkey}" "{self.gmn_voting_address}" ' \
                    f'"{str(round(self.gmn_operator_reward, 2))}" "{self.gmn_owner_payout_addr}" "{addr}"'
            else:
    ...
    def check_tx_confirmation(self):
        try:
            tx = self.geweld_intf.getrawtransaction(self.gmn_reg_tx_hash, 1, skip_cache=True)
    ...
    @pyqtSlot(bool)
    def on_btnSummaryGMNOperatorKeyCopy_clicked(self, checked):
        text = self.edtSummaryGMNOperatorKey.text()
        cl = QApplication.clipboard()
        cl.setText(text)