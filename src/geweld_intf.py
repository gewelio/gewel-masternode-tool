...
log = logging.getLogger('gmt.geweld_intf')
...
class GeweldSSH(object):
    ...
    def find_geweld_config(self):
        """
        Try to read configuration of remote gewel daemon. In particular we need parameters concering rpc
        configuration.
        :return: tuple (geweld_running, geweld_config_found, geweld config file contents as dict)
                or error string in error occured
        """
        geweld_running = False
        geweld_config_found = False
        if not self.ssh:
            raise Exception('SSH session not ready')
        try:
            # find geweld process id if running
            try:
                pids = self.remote_command('ps -C "geweld" -o pid')
            except UnknownError:
                raise Exception('is geweld running on the remote machine?')
            pid = None
            ...
            if pid:
                geweld_running = True
                # using geweld pid find its executable path and then .gewelcore directory and finally gewel.conf file
                ...
                if executables and len(executables) >= 1:
                    ...
                    if len(elems) == 2:
                        ...
                        geweld_dir = os.path.dirname(executable)
                        gewel_conf_file = geweld_dir + '/.gewelcore/gewel.conf'
                        conf_lines = []
                        try:
                            conf_lines = self.remote_command('cat ' + gewel_conf_file)
                        except Exception as e:
                            # probably error no such file or directory
                            # try to read geweld's cwd + cmdline
                            cwd_lines = self.remote_command('ls -l /proc/' + str(pid) + '/cwd')
                            if cwd_lines:
                                elems = cwd_lines[0].split('->')
                                if len(elems) >= 2:
                                    cwd = elems[1]
                                    gewel_conf_file = cwd + '/.gewelcore/gewel.conf'
                                    try:
                                        conf_lines = self.remote_command('cat ' + gewel_conf_file)
                                    except Exception as e:
                                        # second method did not succeed, so assume, that conf file is located
                                        # i /home/<username>/.gewelcore directory
                                        try:
                                            gewel_conf_file = '/home/' + self.username + '/.gewelcore/gewel.conf'
                                            conf_lines = self.remote_command('cat ' + gewel_conf_file)
                                        # If the node was seutp with https://github.com/gewelio/deploy/blob/master/scripts/install-node.sh
                                        # then check gewelcore user location /home/gewelcore/.gewelcore/gewel.conf
                                        # TODO allow user to override conf path
                                        except Exception as e:
                                            gewel_conf_file = '/home/' + 'gewelcore' + '/.gewelcore/gewel.conf'
                                            conf_lines = self.remote_command('cat ' + gewel_conf_file)
                        ...
                        geweld_config_found = True
            return geweld_running, geweld_config_found, config
        except Exception as e:
            return str(e)
...
class GeweldIndexException(JSONRPCException):
    """
    Exception for notifying, that gewel daemon should have indexing option tuned on
    """
    def __init__(self, parent_exception):
        JSONRPCException.__init__(self, parent_exception.error)
        self.message = self.message + \
                       '\n\nMake sure the gewel daemon you are connecting to has the following options enabled in ' \
                       'its gewel.conf:\n\n' + \
                       'addressindex=1\n' + \
                       'spentindex=1\n' + \
                       'timestampindex=1\n' + \
                       'txindex=1\n\n' + \
                       'Changing these parameters requires to execute geweld with "-reindex" option (linux: ./geweld -reindex)'


def control_rpc_call(func):
    """
    Decorator function for catching HTTPConnection timeout and then resetting the connection.
    :param func: GeweldInterface's method decorated
    """
                ...
                try:
                    try:
                        ...
                        if last_conn_reset_time:
                            raise GeweldConnectionError(e)  # switch to another config if possible
                        else:
                            last_exception = e
                        ...
                        # exceptions raised most likely by not functioning geweld node; try to switch to another node
                        # if there is any in the config
                        log.warning('Error while calling of "' + str(func) + ' (3)". Details: ' + str(e))
                        raise GeweldConnectionError(e)

                except GeweldConnectionError as e:
...
class GeweldInterface(WndUtils):
    ...
    def load_data_from_db_cache(self):
        ...
        try:
            ...
            cur.execute("SELECT id, ident, status, protocol, payee, last_seen, active_seconds,"
                        " last_paid_time, last_paid_block, IP from MASTERNODES where gmt_active=1")
    ...
    def switch_to_next_config(self):
        """
        If there is another geweld config not used recently, switch to it. Called only when there was a problem
    ...
    def open(self):
        """
        Opens connection to gewel RPC. If it fails, then the next enabled conn config will be used, if any exists.
        ...
        try:
            if not self.cur_conn_def:
                raise Exception('There is no connections to Gewel network enabled in the configuration.')
            ...
            while True:
                try:
                    ...
                    return False
                        ...
                        NoValidConnectionsError) as e:
                    # exceptions raised by not likely functioning geweld node; try to switch to another node
                    # if there is any in the config
                    if not self.switch_to_next_config():
    ...
    def open_internal(self):
        """
        Try to establish connection to gewel RPC daemon for current connection config.
        ...
        if not self.active:
            ...
            if self.cur_conn_def.use_ssh_tunnel:
                # RPC over SSH
                if self.ssh is None:
                    self.ssh = GeweldSSH(self.cur_conn_def.ssh_conn_cfg.host, self.cur_conn_def.ssh_conn_cfg.port,
        ...
        if self.open():
            ...
            if verify_node:
                ...
                if self.config.is_testnet() and not node_under_testnet:
                    raise Exception('This RPC node works under Gewel MAINNET, but your current configuration is '
                                    'for TESTNET.')
                elif self.config.is_mainnet() and node_under_testnet:
                    raise Exception('This RPC node works under Gewel TESTNET, but your current configuration is '
                                    'for MAINNET.')
            return info
        else:
            raise Exception('Not connected')
    ...
    def issynchronized(self):
        if self.open():
            # if connecting to HTTP(S) proxy do not check if gewel daemon is synchronized
    ...
    def get_masternodelist(self, *args, data_max_age=MASTERNODES_CACHE_VALID_SECONDS) -> List[Masternode]:
        """
        Returns masternode list, read from the Gewel network or from the internal cache.
        ...
        if self.open():
            ...
            if len(args) == 1 and args[0] == 'full':
                last_read_time = app_cache.get_value(f'MasternodesLastReadTime_{self.app_config.gewel_network}', 0, int)
                ...
                if self.masternodes and data_max_age > 0 and \
                   int(time.time()) - last_read_time < data_max_age:
                    ...
                    log.info('Loading masternode list from Gewel daemon...')
                    ...
                    try:
                            ...
                            if not existing_mn:
                                ...
                                if self.db_intf.db_active:
                                    cur.execute("INSERT INTO MASTERNODES(ident, status, protocol, payee, last_seen,"
                                            " active_seconds, last_paid_time, last_paid_block, ip, gmt_active,"
                                            " gmt_create_time) "
                                            "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                        ...
                        for mn_index in reversed(range(len(self.masternodes))):
                            ...
                            if not mn.marker:
                                if self.db_intf.db_active:
                                    cur.execute("UPDATE MASTERNODES set gmt_active=0, gmt_deactivation_time=?"
                                                "WHERE ID=?",
                                                (datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                                mn.db_id))
                                    db_modified = True
                        ...
                        app_cache.set_value(f'MasternodesLastReadTime_{self.app_config.gewel_network}', int(time.time()))
                        self.update_mn_queue_values()
                    finally:
                        if db_modified:
                            self.db_intf.commit()
