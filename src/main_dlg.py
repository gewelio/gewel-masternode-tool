...
from app_config import AppConfig, MasternodeConfig, APP_NAME_SHORT, GMN_ROLE_OWNER, GMN_ROLE_OPERATOR, InputKeyType
from app_defs import PROJECT_URL, HWType, get_note_url
from gewel_utils import bip32_path_n_to_string
from geweld_intf import GeweldInterface, GeweldIndexException
...
class MainWindow(QMainWindow, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def __init__(self, app_dir):
        ...
        self.geweld_intf = GeweldInterface(window=None,
        ...
        self.hw_session = HwSessionInfo(
            ...
            self.config,
            geweld_intf=self.geweld_intf)

        self.geweld_info = {}
        self.is_geweld_syncing = False
        self.geweld_connection_ok = False
        self.connecting_to_geweld = False
    ...
    def setupUi(self):
        ...
        self.wdg_masternode = WdgMasternodeDetails(self, self.app_config, self.geweld_intf)
        ...
        self.geweld_intf.initialize(self.config)
    ...
    def configuration_to_ui(self):
        ...
        self.btnMigrateToGMN.setEnabled(True)
        self.update_edit_controls_state()
    ...
    def display_window_title(self):
        ...
        if self.config.gewel_network == 'TESTNET':
    ...
    def get_project_config_params_thread(self, ctrl, force_check):
        ...
        try:
            response = urllib.request.urlopen(
                'https://raw.githubusercontent.com/gewelio/gewel-masternode-tool/master/app-params.json',
    ...
    def on_action_open_settings_window_triggered(self):
        ...
        if res and dlg.get_is_modified():
            ...
            self.geweld_intf.reload_configuration()
            if gewel_network_sav != self.config.gewel_network or hw_type_sav != self.config.hw_type:
                self.disconnect_hardware_wallet()
                self.config.reset_network_dependent_dyn_params()
            self.display_window_title()
            self.update_edit_controls_state()
    ...
    def show_connection_initiated(self):
        """Shows status information related to a initiated process of connection to a gewel RPC. """
        self.setStatus1Text('<b>RPC network status:</b> trying %s...' % self.geweld_intf.get_active_conn_description(), 'black')
    ...
    def show_connection_failed(self):
        ...
        self.setStatus1Text('<b>RPC network status:</b> failed connection to %s' % self.geweld_intf.get_active_conn_description(), 'red')

    def show_connection_successful(self):
        """Shows status information after successful connetion to a Gewel RPC node."""
        self.setStatus1Text('<b>RPC network status:</b> OK (%s)' % self.geweld_intf.get_active_conn_description(), 'green')

    def show_connection_disconnected(self):
        """Shows status message related to disconnection from Gewel RPC node."""
        self.setStatus1Text('<b>RPC network status:</b> not connected', 'black')

    def connect_gewel_network(self, wait_for_check_finish=False, call_on_check_finished=None):
        """
        Connects do gewel daemon if not connected before and returnes if it was successful.
        ...
        def wait_for_synch_finished_thread(ctrl):
            """
            Thread waiting for gewel daemon to finish synchronizing.
            """
            mtx = QMutex()
            cond = QWaitCondition()
            try:
                ...
                while not ctrl.finish:
                    synced = self.geweld_intf.issynchronized()
                    if synced:
                        self.is_geweld_syncing = False
                        self.show_connection_successful()
                        break
                    mnsync = self.geweld_intf.mnsync()
                    self.setMessage('Geweld is synchronizing: AssetID: %s, AssetName: %s' %
                ...
                mtx.unlock()
                self.wait_for_geweld_synced_thread = None

        def connect_thread(ctrl):
            """
            Test connection to gewel network inside a thread to avoid blocking GUI.
            :param ctrl: control structure to communicate with WorkerThread object (not used here)
            """
            try:
                synced = self.geweld_intf.issynchronized()
                self.geweld_info = self.geweld_intf.getinfo(verify_node=True)
                self.geweld_connection_ok = True
                if not synced:
                    logging.info("geweld not synced")
                    if not self.is_geweld_syncing and not (hasattr(self, 'wait_for_geweld_synced_thread') and
                                                                  self.wait_for_geweld_synced_thread is not None):
                        self.is_geweld_syncing = True
                        self.wait_for_geweld_synced_thread = self.run_thread(self, wait_for_synch_finished_thread, (),
        ...
        def connect_finished():
            ...
            self.connecting_to_geweld = False
            self.config.read_gewel_network_app_params(self.geweld_intf)
        ...
        if self.config.is_config_complete():
            if not hasattr(self, 'check_conn_thread') or self.check_conn_thread is None:
                ...
                if hasattr(self, 'wait_for_geweld_synced_thread') and self.wait_for_geweld_synced_thread is not None:
                    if call_on_check_finished is not None:
                        # if a thread waiting for geweld to finish synchronizing is running, call the callback function
                        call_on_check_finished()
                else:
                    self.connecting_to_geweld = True
    ...
    def on_action_check_network_connection_triggered(self):
        self.test_gewel_network_connection()

    def test_gewel_network_connection(self):
        def connection_test_finished():
            ...
            if self.geweld_connection_ok:
                self.show_connection_successful()
                if self.is_geweld_syncing:
                    self.infoMsg('Connection successful, but Gewel daemon is synchronizing.')
                else:
                    self.infoMsg('Connection successful.')
            else:
                if self.geweld_intf.last_error_message:
                    self.errorMsg('Connection error: ' + self.geweld_intf.last_error_message)
                else:
        ...
        if self.config.is_config_complete():
            ...
            self.btnMigrateToGMN.setEnabled(False)
            self.btnRefreshMnStatus.setEnabled(False)
            self.action_transfer_funds_for_any_address.setEnabled(False)
            self.connect_gewel_network(call_on_check_finished=connection_test_finished)
        else:
        ...
        if not self.hw_client:
            try:
                try:
                    ...
                    if self.config.gewel_network == 'TESTNET':
                        # check if Gewel testnet is supported by this hardware wallet
                        found_testnet_support = False
                        if self.config.hw_type in (HWType.trezor, HWType.keepkey):
                            try:
                                path = gewel_utils.get_default_bip32_base_path(self.config.gewel_network)
                                path += "/0'/0/0"
                                path_n = gewel_utils.bip32_path_string_to_n(path)
                                addr = hw_intf.get_address(self.hw_session, path_n, False)
                                if gewel_utils.validate_address(addr, self.config.gewel_network):
                                    found_testnet_support = True
                            except Exception as e:
                                if str(e).find('Invalid coin name') < 0:
                                    logging.exception('Failed when looking for Gewel testnet support')
                        elif self.config.hw_type == HWType.ledger_nano_s:
                            addr = hw_intf.get_address(self.hw_session,
                                                       gewel_utils.get_default_bip32_path(self.config.gewel_network))
                            if gewel_utils.validate_address(addr, self.config.gewel_network):
                                found_testnet_support = False

                        if not found_testnet_support:
                            url = get_note_url('GMT0002')
                            msg = f'Your hardware wallet device does not support Gewel TESTNET ' \
                ...
                try:
                    with open(file_name, 'r') as f_ptr:
                        bip44_wallet = Bip44Wallet(self.app_config.hw_coin_name, self.hw_session,
                                                   self.app_config.db_intf, self.geweld_intf,
                        ...
                        for line in f_ptr.readlines():
                            ...
                            if len(elems) >= 5 and not line.startswith('#'):
                                ...
                                def update_mn(in_mn):
                                    ...
                                    in_mn.collateralAddress = mn_gewel_addr
                        ...
                        if imported_cnt:
                            ...
                            if self.queryDlg(message=msg_text, buttons=QMessageBox.Yes | QMessageBox.No,
                                ...
                                # scan all Gewel addresses from imported masternodes for BIP32 path, starting from
                                # first standard Gewel BIP32 path
    ...
    def update_edit_controls_state(self):
        def update_fun():
            ...
            self.btnMigrateToGMN.setEnabled(self.cur_masternode is not None)
        if threading.current_thread() != threading.main_thread():
    ...
    def update_mn_controls_state(self):
        if self.cur_masternode:
            vis_non_gmn = self.cur_masternode.gmn_user_roles & GMN_ROLE_OPERATOR > 0 and \
                          self.config.is_non_deterministic_mns_enabled()
            vis_gmn = self.cur_masternode.gmn_user_roles & GMN_ROLE_OPERATOR > 0 and \
                      self.config.is_deterministic_mns_enabled()
        else:
            vis_non_gmn = False
            vis_gmn = False
        self.btnBroadcastMn.setVisible(vis_non_gmn)
        self.btnMigrateToGMN.setVisible(vis_gmn)
    ...
    def create_mn_broadcast_msg(self, mn_protocol_version: int,
                                ...
                                sig_time: int = None) \
            -> gewel_utils.CMasternodeBroadcast:

        if not sig_time:
            sig_time = int(time.time())
        mn_privkey = gewel_utils.wif_to_privkey(masternode.privateKey, self.config.gewel_network)
        if not mn_privkey:
        ...
        mn_broadcast = gewel_utils.CMasternodeBroadcast(
            masternode.ip,
            masternode.port,
            collateral_pubkey,
            mn_pubkey,
            ...
            spork6_active=self.config.get_spork_state_from_config(6, self.config.gewel_network, False)
        )

        signature = mn_broadcast.sign(masternode.collateralBip32Path, hw_intf.hw_sign_message, self.hw_session,
                                      masternode.privateKey, self.config.gewel_network)
    ...
    def on_btnBroadcastMn_clicked(self):
        """
        Broadcasts information about configured Masternode within Gewel network using Hwrdware Wallet for signing message
        and a Gewel daemon for relaying message.
        Building broadcast message is based on work of chaeplin (https://github.com/chaeplin/gewelmnb)
        """
        if self.cur_masternode:
        ...
        self.connect_gewel_network(wait_for_check_finish=True)
        if not self.geweld_connection_ok:
            self.errorMsg("Connection to Gewel daemon is not established.")
            return
        if self.is_geweld_syncing:
            self.warnMsg("The Gewel daemon you are connected to is currently synchronizing. You need wait "
                         "until it's finished.")
            return
        ...
        try:
            mn_privkey = gewel_utils.wif_to_privkey(self.cur_masternode.privateKey, self.config.gewel_network)
            if not mn_privkey:
            ...
            block_count = self.geweld_intf.getblockcount()
            block_count_for_mn_ping = block_count - 12
            block_hash_for_mn_ping = bytes.fromhex(self.geweld_intf.getblockhash(block_count_for_mn_ping))
            ...
            if not cfg_collateral_address:
                ...
                if self.queryDlg(message="The Gewel address retrieved from the hardware wallet (%s) for the configured "
            ...
            try:
                utxos = self.geweld_intf.getaddressutxos([hw_collateral_address])
                found = False
                utxo = []
                for utxo in utxos:
            ...
            except GeweldIndexException as e:
            ...
            info = self.geweld_intf.getinfo(verify_node=True)
            ...
            if ret['overall'].startswith('Successfully decoded broadcast messages for 1 masternodes'):
                ...
                if failed_count == 0:
                    ...
                    self.errorMsg('Failed to start masternode.\n\nResponse from Gewel daemon: %s.' % errorMessage)
            else:
    ...
    def get_masternode_status(self, masternode):
        ...
        if self.geweld_connection_ok:
            ...
            mns_info = self.geweld_intf.get_masternodelist('full', collateral_id)
            if len(mns_info):
    ...
    def get_deterministic_tx(self, masternode: MasternodeConfig) -> Optional[Dict]:
        ...
        if masternode.gmn_tx_hash:
            try:
                protx = self.geweld_intf.protx('info', masternode.gmn_tx_hash)
                if protx:
            ...
            if not protx:
                try:
                    ...
                    tx = self.geweld_intf.getrawtransaction(masternode.gmn_tx_hash, 1, skip_cache=True)
                    confirmations = tx.get('confirmations', 0)
                    if confirmations < 3:
                        # in this case gmn tx should have been found by the 'protx info' call above;
                        # it hasn't been, so it is no longer valid a protx transaction
                        ptx = tx.get('proRegTx')
                        if ptx:
                            protx = {
                                'proTxHash': masternode.gmn_tx_hash,
    ...
    def get_masternode_status_description_thread(self, ctrl, masternode: MasternodeConfig):
        ...
        if self.geweld_connection_ok:
            ...
            self.geweld_intf.get_masternodelist('full', data_max_age=30)  # read new data from the network
                                                                                    # every 30 seconds
            if collateral_id:
                mn_info = self.geweld_intf.masternodes_by_ident.get(collateral_id)
            else:
                mn_info = self.geweld_intf.masternodes_by_ip_port.get(ip_port)

            gmn_tx = self.get_deterministic_tx(masternode)
            if gmn_tx:
                gmn_tx_state = gmn_tx.get('state')
            else:
                gmn_tx_state = {}
            ...
            if mn_info:
                ...
                mn_queue_position = '?'
                if gmn_tx_state:
                    mn_ident = str(gmn_tx.get('collateralHash')) + '-' + str(gmn_tx.get('collateralIndex'))
                    mn_ip_port = gmn_tx_state.get('service')
                else:
                    mn_ident = None
                    mn_ip_port = None

            if mn_info or gmn_tx:
                if mn_info:
                    ...
                    lastseen_str = ''
                    lastseen_ago_str = ''
                    s = gmn_tx_state.get('PoSePenalty', 0)
                    if s:
                ...
                if mn_info:
                    if mn_info.lastpaidtime > time.time() - 3600 * 24 * 365:
                        # fresh gmns have lastpaidtime set to some day in the year 2014
                        lastpaid_ts = mn_info.lastpaidtime
                else:
                    paid_block = gmn_tx_state.get('lastPaidHeight')
                    if paid_block:
                        bh = self.geweld_intf.getblockhash(paid_block)
                        blk = self.geweld_intf.getblockheader(bh, 1)
                ...
                enabled_mns_count = len(self.geweld_intf.payment_queue)
                ...
                if masternode not in self.mns_user_refused_updating:
                    if not masternode.collateralTx or not masternode.collateralTxIndex or \
                            (not masternode.collateralAddress and
                             ((masternode.is_deterministic and masternode.gmn_user_roles & GMN_ROLE_OWNER) or
                              not masternode.is_deterministic)) or \
                            (gmn_tx and masternode.gmn_tx_hash != gmn_tx.get('proTxHash')):
                ...
                if not collateral_tx_mismatch:
                    if (masternode.is_deterministic and masternode.gmn_user_roles & GMN_ROLE_OWNER) or \
                            not masternode.is_deterministic:

                        # check outputs of the collateral transaction
                        tx_json = self.geweld_intf.getrawtransaction(masternode.collateralTx, 1)
                        if tx_json:
                ...
                if gmn_tx:
                    if not masternode.is_deterministic:
                        if update_mn_info:
                            masternode.gmn_tx_hash = gmn_tx.get('proTxHash')
                        ...
                        gmn_hash = gmn_tx.get('proTxHash')
                        if gmn_hash and masternode.gmn_tx_hash != gmn_hash:
                            if update_mn_info:
                                masternode.gmn_tx_hash = gmn_tx.get('proTxHash')
                                mn_data_modified = True

                    if gmn_tx_state:
                        owner_address_network = gmn_tx_state.get('ownerAddress')
                        owner_address_cfg = masternode.get_gmn_owner_public_address(self.app_config.gewel_network)
                        if owner_address_network and owner_address_cfg and owner_address_network != owner_address_cfg:
                            ...
                            logging.warning(
                                ...
                                f'address from the app configuration: {owner_address_cfg}, address from the Gewel '
                                f'network: {owner_address_network}')

                        voting_address_network = gmn_tx_state.get('votingAddress')
                        voting_address_cfg = masternode.get_gmn_voting_public_address(self.app_config.gewel_network)
                        if voting_address_network and voting_address_cfg and voting_address_network != voting_address_cfg:
                            ...
                            logging.warning(
                                ...
                                f'address from the app configuration: {voting_address_cfg}, address from the Gewel '
                                f'network: {voting_address_network}')

                        operator_pubkey_network = gmn_tx_state.get('pubKeyOperator')
                        operator_pubkey_cfg = masternode.get_gmn_operator_pubkey()
                        if operator_pubkey_network and operator_pubkey_cfg and operator_pubkey_network != operator_pubkey_cfg:
                            ...
                            logging.warning(
                                ...
                                f'pubkey from the app configuration: {operator_pubkey_cfg}, pubkey from the Gewel '
                ...
                if masternode == self.cur_masternode:
                    ...
                    if addr:
                        try:
                            bal = self.geweld_intf.getaddressbalance([addr])
                            if bal:
                    ...
                    errors = []
                    warnings  = []
                    if gmn_tx and not gmn_tx.get('confirmations'):
                    ...
                    if owner_public_address_mismatch:
                        errors.append('<td class="error" colspan="2">Owner Gewel address mismatch</td>')
                    ...
                    if voting_public_address_mismatch:
                        errors.append('<td class="error" colspan="2">Voting Gewel address mismatch</td>')
            ...
            status = '<span style="color:red">Problem with connection to geweld.</span>'
    ...
    def on_btnRefreshMnStatus_clicked(self):
        ...
        if self.geweld_connection_ok:
            ...
            enable_buttons()
            self.lblMnStatus.setText('')
            self.errorMsg('Gewel daemon not connected')
    ...
    def on_action_transfer_funds_for_cur_mn_triggered(self):
        ...
        if self.cur_masternode:
            ...
            if not self.cur_masternode.collateralBip32Path:
                ...
                self.errorMsg("Enter the masternode collateral Gewel address. You can use the 'left arrow' "
    ...
    def on_action_open_proposals_window_triggered(self):
        ui = ProposalsDlg(self, self.geweld_intf)
        ui.exec_()
    ...
    @pyqtSlot(bool)
    def on_btnMigrateToGMN_clicked(self, enabled):
        reg_dlg = None

        def on_proregtx_finished(masternode: MasternodeConfig):
            nonlocal reg_dlg, self
            try:
                if self.cur_masternode.gmn_tx_hash != reg_dlg.gmn_reg_tx_hash or \
                        self.cur_masternode.gmn_owner_key_type != reg_dlg.gmn_owner_key_type or \
                        (self.cur_masternode.gmn_owner_key_type == InputKeyType.PRIVATE and
                         self.cur_masternode.gmn_owner_private_key != reg_dlg.gmn_owner_privkey) or \
                        (self.cur_masternode.gmn_owner_key_type == InputKeyType.PUBLIC and
                         self.cur_masternode.gmn_owner_address != reg_dlg.gmn_owner_address) or \
                        self.cur_masternode.gmn_operator_key_type != reg_dlg.gmn_operator_key_type or \
                        (self.cur_masternode.gmn_operator_key_type == InputKeyType.PRIVATE and
                         self.cur_masternode.gmn_operator_private_key != reg_dlg.gmn_operator_privkey) or \
                        (self.cur_masternode.gmn_operator_key_type == InputKeyType.PUBLIC and
                         self.cur_masternode.gmn_operator_public_key != reg_dlg.gmn_operator_pubkey) or \
                        self.cur_masternode.gmn_voting_key_type != reg_dlg.gmn_voting_key_type or \
                        (self.cur_masternode.gmn_voting_key_type == InputKeyType.PRIVATE and
                         self.cur_masternode.gmn_voting_private_key != reg_dlg.gmn_voting_privkey) or \
                        (self.cur_masternode.gmn_voting_key_type == InputKeyType.PUBLIC and
                         self.cur_masternode.gmn_voting_address != reg_dlg.gmn_voting_address) or \
                        not self.cur_masternode.is_deterministic:

                    self.cur_masternode.gmn_tx_hash = reg_dlg.gmn_reg_tx_hash

                    self.cur_masternode.gmn_owner_key_type = reg_dlg.gmn_owner_key_type
                    if self.cur_masternode.gmn_owner_key_type == InputKeyType.PRIVATE:
                        self.cur_masternode.gmn_owner_private_key = reg_dlg.gmn_owner_privkey
                    else:
                        self.cur_masternode.gmn_owner_address = reg_dlg.gmn_owner_address
                        self.cur_masternode.gmn_owner_private_key = ''

                    self.cur_masternode.gmn_operator_key_type = reg_dlg.gmn_operator_key_type
                    if self.cur_masternode.gmn_operator_key_type == InputKeyType.PRIVATE:
                        self.cur_masternode.gmn_operator_private_key = reg_dlg.gmn_operator_privkey
                    else:
                        self.cur_masternode.gmn_operator_public_key = reg_dlg.gmn_operator_pubkey
                        self.cur_masternode.gmn_operator_private_key = ''

                    self.cur_masternode.gmn_voting_key_type = reg_dlg.gmn_voting_key_type
                    if self.cur_masternode.gmn_voting_key_type == InputKeyType.PRIVATE:
                        self.cur_masternode.gmn_voting_private_key = reg_dlg.gmn_voting_privkey
                    else:
                        self.cur_masternode.gmn_voting_address = reg_dlg.gmn_voting_address
                        self.cur_masternode.gmn_voting_private_key = ''
        ...
        if self.cur_masternode:
            reg_dlg = reg_masternode_dlg.RegMasternodeDlg(self, self.app_config, self.geweld_intf, self.cur_masternode,
