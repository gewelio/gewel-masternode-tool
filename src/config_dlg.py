...
import app_config
import app_cache
from app_config import AppConfig, GewelNetworkConnectionCfg
...
class ConfigDlg(QDialog, Ui_ConfigDlg, WndUtils):
    def __init__(self, parent, config):
        ...
        # list of connections from self.local_config.gewel_net_configs split on separate lists for mainnet and testnet
        self.connections_mainnet = []
        self.connections_testnet = []
        self.connections_current = None
        self.current_network_cfg : Optional[GewelNetworkConnectionCfg] = None
    ...
    def setupUi(self):
        ...
        if self.local_config.is_mainnet():
            self.cboGewelNetwork.setCurrentIndex(0)
            self.connections_current = self.connections_mainnet
        else:
            self.cboGewelNetwork.setCurrentIndex(1)
            self.connections_current = self.connections_testnet
        for cfg in self.local_config.gewel_net_configs:
            if cfg.testnet:
        ...
        note_url = get_note_url('GMTN0001')
        ...
        if len(self.local_config.gewel_net_configs):
    ...
    @pyqtSlot(int)
    def on_cboGewelNetwork_currentIndexChanged(self, index):
        """Executed after changing configuration between MAINNET and TESTNET."""
        if not self.disable_cfg_update:
            if index == 0:
                self.connections_current = self.connections_mainnet
                self.local_config.gewel_network = 'MAINNET'
            else:
                self.connections_current = self.connections_testnet
                self.local_config.gewel_network = 'TESTNET'
            self.display_connection_list()
            self.set_modified()
            self.lstConns.setCurrentRow(0)
        ...
        try:
            ...
            if isinstance(conns, list) and len(conns):
                ...
                if self.queryDlg('Do you really want to import connection(s) from clipboard?',
                    ...
                    self.local_config.gewel_net_configs.clear()
                    self.local_config.gewel_net_configs.extend(self.connections_mainnet)
                    self.local_config.gewel_net_configs.extend(self.connections_testnet)

                    added, updated = self.local_config.import_connections(
                        conns, force_import=True, limit_to_network=self.local_config.gewel_network)
                    for cfg in added:
                        cfg.enabled = True
    ...
    def on_btnRestoreDefault_clicked(self):
        if self.queryDlg('Do you really want to restore default connection(s)?',
                         buttons=QMessageBox.Yes | QMessageBox.Cancel,
                         default_button=QMessageBox.Yes, icon=QMessageBox.Information) == QMessageBox.Yes:
            cfgs = self.local_config.decode_connections(default_config.geweld_default_connections)
            if cfgs:
                # update the main list containing connections configuration from separate  lists dedicated
                # to mainnet and testnet - it'll be used by the import_connection method
                self.local_config.gewel_net_configs.clear()
                ...
                added, updated = self.local_config.import_connections(
                    cfgs, force_import=True, limit_to_network=self.local_config.gewel_network)
                self.connections_current.extend(added)
                if added or updated:
                    row_selected = self.lstConns.currentRow()
                    self.display_connection_list()
    ...
    def on_action_new_connection_triggered(self):
        ...
        cfg.testnet = True if self.cboGewelNetwork.currentIndex() == 1 else False
    ...
    def on_chbRandomConn_toggled(self, checked):
        if not self.disable_cfg_update:
            self.local_config.random_gewel_net_config = checked
            self.set_modified()
    ...
    def update_connection_details_ui(self):
        ...
        try:
            ...
            self.chbRandomConn.setChecked(self.local_config.random_gewel_net_config)
        finally:
    ...
    def on_btnSshReadRpcConfig_clicked(self):
        """Read the configuration of a remote RPC node from the node's gewel.conf file."""
        if self.current_network_cfg:
            ...
            if not ok or not username:
                return
            from geweld_intf import GeweldSSH
            ssh = GeweldSSH(host, int(port), username)
            try:
                ssh.connect()
                geweld_conf = ssh.find_geweld_config()
                self.disable_cfg_update = True
                if isinstance(geweld_conf, tuple) and len(geweld_conf) >= 3:
                    if not geweld_conf[0]:
                        self.infoMsg('Remore Gewel daemon seems to be shut down')
                    elif not geweld_conf[1]:
                        self.infoMsg('Could not find remote geweld.conf file')
                    else:
                        file = geweld_conf[2]
                        ...
                        if file.get('server', '1') == '0':
                            self.warnMsg("Remote gewel.conf parameter 'server' is set to '0', so RPC interface will "
                                         "not work.")
                        if not rpcuser:
                            self.warnMsg("Remote gewel.conf parameter 'rpcuser' is not set, so RPC interface will  "
                                         "not work.")
                        if not rpcpassword:
                            self.warnMsg("Remote gewel.conf parameter 'rpcpassword' is not set, so RPC interface will  "
                                         "not work.")
                    self.update_connection_details_ui()
                elif isinstance(geweld_conf, str):
                    self.warnMsg("Couldn't read remote geweld configuration file due the following error: " +
                                 geweld_conf)
                ssh.disconnect()
            except Exception as e:
                self.errorMsg(str(e))
    ...
    def on_btnTestConnection_clicked(self):
        if self.current_network_cfg:
            ...
            geweld_intf = GeweldInterface(window=self)
            geweld_intf.initialize(self.local_config, connection=self.current_network_cfg,
                                  for_testing_connections_only=True)
            try:
                info = geweld_intf.getinfo(verify_node=True)
                if info:
